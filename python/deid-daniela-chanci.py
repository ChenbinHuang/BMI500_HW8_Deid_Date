# Modified by: Daniela Chanci Arrubla
# Date: 10/18/2021
...
# define case-insensitive regex
prefix_pattern = "(?i)(W\/|\s)dr\.?\s"

# compiling the reg_ex would save sime time!
ph_reg = re.compile(prefix_pattern)


def check_for_HCPName(patient, note, chunk, tokens, output_handle):
    """
    Inputs:
        ...
        - tokens: list of words contained in the record of a patient. Obtained with split().
    ...
    for match in ph_reg.finditer(chunk):
        ...
        # find the index of the match in the list of words
        match_index = tokens.index(match.group().strip())

        # assign to name the element in the list in the position match_index + 1
        name = tokens[match_index+1]

        # discard commas and periods that are located right after the name
        if "." in name:
            name = name.split(".")[0]
        elif "," in name :
            name = name.split(",")[0]
        ...
        result = str(match.end() - offset) + ' ' + str(match.end() - offset) + ' ' + str(match.end() + len(name) - offset)
...
def deid_HCPName(text_path='id.text', output_path='HCPName-daniela-chanci.phi'):
    """
    Inputs:
        ...
        with open(text_path) as text:
            ...
            for line in text:
                ...
                if len(record_end):
                    ...
                    check_for_HCPName(patient, note, chunk.strip(), chunk.strip().split(), output_file)
